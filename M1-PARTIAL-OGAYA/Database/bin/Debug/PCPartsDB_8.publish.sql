...
BEGIN
  ...
  SET @sql = N'SELECT @TotalPrice = @TotalPrice + [UnitPrice] FROM [dbo].[' + QUOTENAME(@PCPart) + '] WHERE [Id] = @Id;';
  EXEC sp_executesql @sql, N'@Id INT, @TotalPrice DECIMAL(10, 2) OUTPUT', @Id = @Id, @TotalPrice = @TotalPrice OUTPUT;

  SET @sql = N'SELECT [Name], @TotalPrice AS [UnitPrice] FROM [dbo].[' + QUOTENAME(@PCPart) + '] WHERE [Id] = @Id;';
  EXEC sp_executesql @sql, N'@Id INT, @TotalPrice DECIMAL(10, 2)', @Id = @Id, @TotalPrice = @TotalPrice;
END
GO
